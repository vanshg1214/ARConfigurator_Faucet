...
export const tapPlaceComponent = {
  ...
  init() {
      ...
      g.addEventListener('click', (event) => {
        ...
        if (this.faucetElement) {
          ...
          this.faucetElement.setAttribute('animation__pos', {
            property: 'position',
            to: `${touchPoint.x} ${touchPoint.y + 3.90} ${touchPoint.z}`,
            easing: 'easeOutQuad',
            dur: 500,
          })
          return
        ...
        newElement.setAttribute('position', `${touchPoint.x} ${touchPoint.y + 3.90} ${touchPoint.z}`)
        ...
        newElement.addEventListener('model-loaded', () => {
          ...
          if (obj) {
            ...
            obj.traverse((node) => {
              // Grab every valid node/mesh for the debugger
              if (node.name && node.type !== 'Bone' && node.type !== 'Scene') {
                node.matrixAutoUpdate = true
                node.userData.originalZ = node.position.z
                node.userData.targetOffsetZ = 0
          ...
          const partOffsetSlider = document.getElementById('partOffsetSlider')
          const partOffsetInput = document.getElementById('partOffsetInput')
          const btnPrintOffsets = document.getElementById('btnPrintOffsets')

          if (debugPanel && partSelect) {
            debugPanel.style.display = 'block'
            
            // Populate Dropdown
            partSelect.innerHTML = ''
            this.explosionNodes.forEach((node, i) => {
            ...
            partSelect.addEventListener('change', (e) => {
              ...
              const selectedNode = this.explosionNodes[idx]
              partOffsetSlider.value = selectedNode.userData.targetOffsetZ
              if (partOffsetInput) partOffsetInput.value = selectedNode.userData.targetOffsetZ.toFixed(3)
            })

            // Wire Slider
            partOffsetSlider.addEventListener('input', (e) => {
              const val = parseFloat(e.target.value)
              if (partOffsetInput) partOffsetInput.value = val.toFixed(3)
              const idx = parseInt(partSelect.value)
              const selectedNode = this.explosionNodes[idx]
              selectedNode.userData.targetOffsetZ = val
              
              // Force Explode mode ON if they move the slider
              if (!this.isExploded) {
                const btnExplode = document.getElementById('btnExplode')
                if (btnExplode) btnExplode.click()
              }
            })

            // Wire Number Input
            if (partOffsetInput) {
              partOffsetInput.addEventListener('input', (e) => {
                let val = parseFloat(e.target.value)
                if (isNaN(val)) return // Ignore invalid typing until valid
                partOffsetSlider.value = val
                const idx = parseInt(partSelect.value)
                const selectedNode = this.explosionNodes[idx]
                selectedNode.userData.targetOffsetZ = val
            ...
            // Wire Print Button
            btnPrintOffsets.addEventListener('click', (e) => {
              e.stopPropagation()
              const tableData = this.explosionNodes.map(n => ({
                Name: n.name,
                Offset: n.userData.targetOffsetZ.toFixed(3)
              }))
              console.table(tableData)
              alert('Printed to console! Press F12 to view.')
            })
          }
        })
      })
    ...
    if (btnExplode) {
      btnExplode.addEventListener('click', (e) => {
        ...
        if (explodeBtnText) {
          explodeBtnText.innerText = this.isExploded ? 'Assemble' : `Explode (${this.explosionNodes.length})`
        }
        
        // Turn off water if exploding to prevent weird visuals
        if (this.isExploded && this.waterActive && btnWater) {
          btnWater.click()
        }
      })
    }
  ...
  tick(time, timeDelta) {
    ...
    // Animate exploded parts
    if (this.explosionNodes && this.explosionNodes.length > 0) {
      const explodeLerp = 1 - Math.pow(0.01, timeDelta / 1000)
      this.explosionNodes.forEach(node => {
        const targetZ = node.userData.originalZ + (this.isExploded ? node.userData.targetOffsetZ : 0)
        node.position.z += (targetZ - node.position.z) * explodeLerp
        node.updateMatrix() // Force matrix update just in case
      })
    }
